rule multiple_imputation_plink2:
    ...
    input:
        lambda wildcards: expand(
            "results/{{analysis}}/{{tool}}/mi_runs/{runnum}/results.{{pheno_name}}.glm.{suffix}",
...
rule run_plink2_linear_regression:
    ...
    """
    input:
        vcf="results/{analysis}/{tool}/mi_runs/{runnum}/data.vcf.gz",
    output:
        filename="results/{analysis}/{tool}/mi_runs/{runnum}/results.{pheno_name}.glm.linear",
        log=temp("results/{analysis}/{tool}/mi_runs/{runnum}/results.{pheno_name}.log"),
    conda:
        "../envs/plink2.yaml"
    params:
        plink2_memlimit=config["plink2"]["memlimit"],
        plink2_outprefix="results/{analysis}/{tool}/mi_runs/{runnum}/results",
...
use rule run_plink2_linear_regression as run_plink2_logistic_regression with:
    output:
        filename="results/{analysis}/{tool}/mi_runs/{runnum}/results.{pheno_name}.glm.logistic.hybrid",
        log=temp("results/{analysis}/{tool}/mi_runs/{runnum}/results.{pheno_name}.log"),
    params:
        memlimit=config["tools"]["plink"]["memlimit"] / 4,
        plink2_outprefix="results/{analysis_name}/{analysis}/{tool}/{prefix}",
