...
void MI::fileinterface_writer_bzip2::open(const char *filename) {
  if (_raw_output)
    throw std::domain_error("MI::fileinterface_writer_bzip2: attempted to reopen "
			    "in-use handle");
  _raw_output = fopen(filename, "w");
  if (!_raw_output)
    throw std::domain_error("MI::fileinterface_writer_bzip2: cannot open file \""
			    + std::string(filename) + "\"");
  int error = 0;
  _bz_output = BZ2_bzWriteOpen(&error, _raw_output, 9, 0, 0);
  if (error == BZ_CONFIG_ERROR) {
    throw std::domain_error("MI::fileinterface_writer_bzip2::open: bzip2 writing "
...
void MI::fileinterface_writer_bzip2::close() {
  int error = 0;
  if (_bz_output) {
    BZ2_bzWriteClose(&error, _bz_output, 0, 0, 0);
    if (error == BZ_SEQUENCE_ERROR) {
      throw std::domain_error("MI::fileinterface_writer_bzip2::close: bzip reports "
...
void MI::fileinterface_writer_bzip2::clear() {
  _good = true;
  _bad = _fail = false;
}

bool MI::fileinterface_writer_bzip2::is_open() const {
  return (_raw_output && _bz_output);
}

void MI::fileinterface_writer_bzip2::put(char c) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR ||
    ...
    throw std::domain_error("MI::fileinterface_writer_bzip2::put: bzip reports "
			    "write operation called on read handle");
  }
}

void MI::fileinterface_writer_bzip2::writeline(const std::string &orig_line) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR ||
    ...
    throw std::domain_error("MI::fileinterface_writer_bzip2::writeline: bzip reports "
			    "write operation called on read handle");
  }
}

void MI::fileinterface_writer_bzip2::write(char *buf, std::streamsize n) {
  int error = 0;
  ...
  if (error == BZ_PARAM_ERROR ||
    ...
    throw std::domain_error("MI::fileinterface_writer_bzip2::write: bzip reports "
			    "write operation called on read handle");
  }
}
