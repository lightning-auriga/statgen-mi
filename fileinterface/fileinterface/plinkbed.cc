...
MI::plinkbed_reader::plinkbed_reader(unsigned line_length,
...
void MI::plinkbed_reader::open(const char *filename) {
  if (_input)
    throw std::domain_error("MI::plinkbed_reader::open: reopen attempted "
			    "on active handle");
  if (_compress == GZIP) {
#ifdef FILEINTERFACE_HAVE_LIBZ
    _input = new MI::fileinterface_reader_gzip;
#else
    ...
    _input = new MI::fileinterface_reader_bzip2;
#else
    ...
    _input = new MI::fileinterface_reader_flat;
  }
  ...
  if (buf[0] != (char)108 ||
      buf[1] != (char)27 ||
      buf[2] != (char)1)
    throw std::domain_error("MI::plinkbed_reader::open: encountered unrecognized header data");
}

void MI::plinkbed_reader::close() {
  if (_input) {
    _input->close();
    delete _input;
    _input = 0;
...
void MI::plinkbed_reader::clear() {
  if (_input)
    _input->clear();
}

bool MI::plinkbed_reader::is_open() const {
  if (_input) return _input->is_open();
  return false;
}

char MI::plinkbed_reader::get() {
  //probably not a thing that should really be allowed
  throw std::domain_error("MI::plinkbed_reader::get: functionality "
			  "disabled for bed files");
}

bool MI::plinkbed_reader::getline(std::string &line) {
  //return it in format "120100012212012211200120012"....
  //coded allele is allele 1
  if (!_input)
    throw std::domain_error("MI::plinkbed_reader::getline: called on unopened file");
...
bool MI::plinkbed_reader::eof() const {
  if (_input) return _input->eof();
  return false;
}

bool MI::plinkbed_reader::good() const {
  if (_input) return _input->good();
  return true;
}

bool MI::plinkbed_reader::bad() const {
  if (_input) return _input->bad();
  return false;
}

void MI::plinkbed_reader::read(char *buf, std::streamsize n) {
  if (_input) {
    _input->read(buf, n);
  } else {
    throw std::domain_error("MI::plinkbed_reader::read: called before file open");
  }
}

MI::plinkbed_writer::plinkbed_writer(const std::string &compression_level)
				     
  : MI::fileinterface_writer(),
    _output(0),
    _buf(0),
    _line_length(0),
    _line_length_packed(0) {
...
void MI::plinkbed_writer::open(const char *filename) {
  if (_output)
    throw std::domain_error("MI::plinkbed_writer::open: reopen attempted "
			    "on active handle");
  if (_compress == GZIP) {
#ifdef FILEINTERFACE_HAVE_LIBZ
    _output = new MI::fileinterface_writer_gzip;
#else
...
#ifdef FILEINTERFACE_HAVE_LIBBZ2
    _output = new MI::fileinterface_writer_bzip2;
#else
    ...
    _output = new MI::fileinterface_writer_flat;
  }
...
void MI::plinkbed_writer::clear() {
  if (_output)
    _output->clear();
}

bool MI::plinkbed_writer::is_open() const {
  if (_output) return _output->is_open();
  return false;
}

void MI::plinkbed_writer::put(char c) {
  throw std::domain_error("MI::plinkbed_writer::put: function disabled for plink bed");
}

void MI::plinkbed_writer::writeline(const std::string &line) {
  if (!_output)
    throw std::domain_error("MI::plinkbed_writer::writeline: called on unopened file");
  ...
  if (_line_length == _line_length_packed &&
    ...
    if (_buf)
      throw std::domain_error("MI::plinkbed_writer::writeline: buffer allocated with 0 size?");
    _buf = new char[_line_length_packed];
  }

  
  if (line.size() != _line_length)
    throw std::domain_error("MI::plinkbed_writer::writeline: provided line does not match "
...
void MI::plinkbed_writer::write(char *buf, std::streamsize n) {
  if (_output) {
    _output->write(buf, n);
  } else {
    throw std::domain_error("MI::plinkbed_writer::write: called before file open");
  }
}

bool MI::plinkbed_writer::eof() const {
  if (_output) return _output->eof();
  return false;
}

bool MI::plinkbed_writer::good() const {
  if (_output) return _output->good();
  return true;
}

bool MI::plinkbed_writer::fail() const {
  if (_output) return _output->fail();
  return false;
}

bool MI::plinkbed_writer::bad() const {
